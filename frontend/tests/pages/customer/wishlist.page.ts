...
export class CustomerWishlistPage extends CustomerBasePage {
  ...
  async moveItemToCart(title: string): Promise<void> {
    ...
    await Promise.all([
      this.page.waitForResponse((response) => response.url().includes('/move-to-cart') && response.request().method() === 'POST'),
  ...
  async removeItem(title: string): Promise<void> {
    ...
    await Promise.all([
      this.page.waitForResponse((response) => response.url().includes('/api/v1/wishlist/') && response.request().method() === 'DELETE'),

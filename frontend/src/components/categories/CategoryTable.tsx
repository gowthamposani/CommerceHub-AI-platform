import { Eye, Pencil, Power, PowerOff, Trash2 } from "lucide-react";
import type { ReactNode } from "react";
import { Link } from "react-router-dom";

import { EmptyState } from "@/components/common/EmptyState";
import { Badge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/Button";
import { TableSkeleton } from "@/components/table/TableSkeleton";
import type { Category } from "@/types/category";
import { formatDate } from "@/utils/formatters";

function statusTone(status: Category["status"]) {
  if (status === "active") return "success";
  if (status === "inactive") return "warning";
  return "neutral";
}

export function CategoryTable({
  categories,
  parentNameById,
  loading,
  onActivate,
  onDeactivate,
  onDelete,
  emptyAction
}: {
  categories: Category[];
  parentNameById: Map<string, string>;
  loading?: boolean;
  onActivate: (category: Category) => void;
  onDeactivate: (category: Category) => void;
  onDelete: (category: Category) => void;
  emptyAction?: ReactNode;
}) {
  if (loading) {
    return <TableSkeleton rows={6} />;
  }

  if (categories.length === 0) {
    return (
      <div className="space-y-4">
        <EmptyState title="No Categories Found" message="Create a category to start organizing product catalogs." />
        {emptyAction ? <div className="flex justify-center">{emptyAction}</div> : null}
      </div>
    );
  }

  return (
    <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-soft">
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="sticky top-0 bg-gray-50">
            <tr>
              {["Category Name", "Parent Category", "Slug", "Status", "Created Date", "Actions"].map((header) => (
                <th
                  key={header}
                  className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-gray-500"
                >
                  {header}
                </th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 bg-white">
            {categories.map((category) => (
              <tr key={category.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 text-sm font-bold text-gray-950">{category.category_name}</td>
                <td className="px-4 py-3 text-sm text-gray-700">
                  {category.parent_category_id
                    ? (parentNameById.get(category.parent_category_id) ?? "Unknown")
                    : "Root"}
                </td>
                <td className="px-4 py-3 text-sm text-gray-700">{category.category_slug}</td>
                <td className="px-4 py-3">
                  <Badge tone={statusTone(category.status)}>{category.status}</Badge>
                </td>
                <td className="px-4 py-3 text-sm text-gray-700">{formatDate(category.created_at)}</td>
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1">
                    <Link to={`/categories/${category.id}`}>
                      <Button variant="ghost" size="icon" aria-label="View category">
                        <Eye className="h-4 w-4" />
                      </Button>
                    </Link>
                    <Link to={`/categories/${category.id}/edit`}>
                      <Button variant="ghost" size="icon" aria-label="Edit category">
                        <Pencil className="h-4 w-4" />
                      </Button>
                    </Link>
                    {category.is_active ? (
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => onDeactivate(category)}
                        aria-label="Deactivate category"
                      >
                        <PowerOff className="h-4 w-4" />
                      </Button>
                    ) : (
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => onActivate(category)}
                        aria-label="Activate category"
                      >
                        <Power className="h-4 w-4" />
                      </Button>
                    )}
                    <Button variant="ghost" size="icon" onClick={() => onDelete(category)} aria-label="Delete category">
                      <Trash2 className="h-4 w-4 text-brand-red" />
                    </Button>
                  </div>
                </td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
